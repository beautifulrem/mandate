...
一份可撤销的 AI 投票摄政权。主权始终在你:授出一份受限的投票摄政,看它替你治理,
随时一键在链上收回整条委托链。
...
- 真实的代理间衰减:用户、编排器、分析师三级,每一跳权限严格更窄,且在兑付时由链上校验,
  而不是靠文档承诺。
...
DAO 配置一次「投票 gas 账户」,成员从此不用考虑 gas。赞助方补贴的是参与而不是立场:它在
...
| 存在的意义 | 评委可以免费、几分钟内亲手驱动每个机制 | 证明同一套机制在真金白银的条件下依然成立 |

同一个产品,直到铸票前是同一条代码路径。测试网证明它可触摸,主网证明它是真的。提示:打开
...
一句话定位:一份常设、仅投票、可撤销的 AI 治理委托。代理替你对 DAO 任意提案投票,可验证
地碰不到你的资金,而你能一键在链上收回整条委托链。下面的 A2A 再委托、Venice TEE、x402 和
1Shot 是把这件事做实的机制。
...
| 可撤销委托 + 撕链(核心):撤销禁用根委托,下一次兑付 revert | live | disable UserOp [`0x1475…c74b`](https://sepolia.basescan.org/tx/0x147517e3b3120bb2bc60ee98a0de2017b4d4412ad9cbf58d06954a8e4d4dc74b);`canRedeem` 由 true 翻为 false;复现 `pnpm revoke:2hop` |
