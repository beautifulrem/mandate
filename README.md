...
<img src="docs/assets/logo.png" width="96" alt="Mandate: a faceted governance check that begins in a chain link and carries a removable locking pin (the on-chain kill switch)">
...
| **Mandate (ERC-7710 scoped + revocable)** | ✅ `AllowedMethods`=castVote · `AllowedTargets`=this board | ✅ `LimitedCalls` + `Timestamp` caveats | ✅ `disableDelegation` cascade-revokes the chain | ✅ the EVM enforces it; the agent can't override |
...
The app ships both networks deliberately; each answers a different question:
...
| Who casts · who pays gas | the analyst redeems the chain and pays **testnet ETH** (free faucet) | the **1Shot relayer** redeems, and **nobody holds ETH**: the burner sponsors the fee in USDC, 1Shot fronts the real gas |
