...
const en = {
  ...
  tabs: { tally: 'Tally', x402: 'x402', oneShot: '1Shot', run: 'Run', noRun: 'No active run yet — grant a mandate to start.' },
  ...
  oneShotReplay: 'replayed lifecycle',
  ...
  x402: {
    ...
    result: 'Settled: the seller pulled EXACTLY 1 MVOTE through the scoped delegation — nothing more was reachable. Its live balance below is the real proof; the feed then returns 200 + data.',
    phases: { require: '402 Required', sign: 'Sign scope', settle: 'Seller redeems', data: '200 Data' },
    phaseDesc: {
      require: 'The data feed answers the analyst with 402 — pay before any data is served.',
      sign: 'The buyer signs a scoped Erc20TransferAmount delegation: at most the toll, payable only to the seller.',
      settle: 'The seller redeems it, pulling exactly 1 MVOTE from the buyer’s smart account — and nothing else.',
...
const zh: Dict = {
  ...
  tabs: { tally: '计票', x402: 'x402', oneShot: '1Shot', run: '运行', noRun: '暂无运行 —— 先授予一个授权。' },
  ...
  x402: {
    ...
    liveRead: '实时链上',
    result: '已结算:卖方凭受限委托精确划走 1 MVOTE —— 别的一概取不走。下方实时余额即为真实凭证;随后数据源返回 200 与内容。',
    phases: { require: '402 需付款', sign: '签署范围', settle: '卖方 redeem', data: '200 数据' },
    phaseDesc: {
      require: '数据源对分析员的请求回以 402:取数据前先付费。',
      sign: '买方签署受限的 Erc20TransferAmount 委托:至多通行费、且只能付给卖方。',
      settle: '卖方兑现委托,从买方智能账户精确划走 1 MVOTE,多一分都取不走。',
